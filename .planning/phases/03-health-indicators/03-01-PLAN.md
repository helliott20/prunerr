...
must_haves:
  ...
  artifacts:
    ...
    - path: "server/src/services/init.ts"
      provides: "Singleton getters for all 5 services"
      exports: ["getPlexService", "getTautulliService", "getSonarrService", "getRadarrService", "getOverseerrService"]
  key_links:
    - from: "server/src/routes/health.ts"
      to: "services/init.ts"
      via: "getPlexService(), getTautulliService(), etc."
      pattern: "get(Plex|Tautulli|Sonarr|Radarr|Overseerr)Service"
    - from: "server/src/routes/health.ts"
...
@server/src/routes/health.ts
@server/src/scheduler/index.ts
@server/src/services/init.ts (existing pattern for service singletons)
@server/src/services/plex.ts (testConnection pattern)
@server/src/services/tautulli.ts (testConnection pattern)
...
<task type="auto">
  <name>Task 2: Add Plex and Tautulli service getters to init.ts</name>
  <files>server/src/services/init.ts</files>
  <action>
Add singleton getter functions for PlexService and TautulliService following the existing pattern in init.ts.

1. Add imports at top of file (after existing service imports):
```typescript
import { PlexService } from './plex';
import { TautulliService } from './tautulli';
```

2. Add singleton variables (after existing `let overseerrService` line ~15):
```typescript
let plexService: PlexService | null = null;
let tautulliService: TautulliService | null = null;
```

3. Add a separate config helper for Plex (since it uses 'token' not 'apiKey'):
```typescript
/**
...
4. Add getter function for Plex (after getOverseerrService):
```typescript
/**
...
5. Add getter function for Tautulli (after getPlexService):
```typescript
/**
...
6. Update refreshServices() to also clear Plex and Tautulli:
```typescript
...
7. Update the default export at bottom of file to include new getters:
```typescript
export default {
  initializeServices,
...
cd server && npx tsc --noEmit
```
Should compile without errors. Verify exports exist:
```bash
grep -E "export function get(Plex|Tautulli)Service" server/src/services/init.ts
```
Should show both function exports.
  </verify>
  <done>getPlexService() and getTautulliService() exported from init.ts, following existing singleton pattern</done>
</task>

<task type="auto">
  <name>Task 3: Create aggregated health status endpoint</name>
...
2. getPlexService() and getTautulliService() exported from server/src/services/init.ts
3. GET /api/health/status returns JSON with services array, scheduler object, and overall status
4. Each service in array has: service name, configured boolean, connected boolean, optional responseTimeMs/error
5. Scheduler object has: isRunning, lastScan (ISO string or null), nextRun (ISO string or null), scanSchedule
6. Response time for endpoint < 10 seconds (parallel execution, not 5*5s sequential)
</verification>

<success_criteria>
- cron-parser installed and used for accurate next-run calculation
- getPlexService() and getTautulliService() available in init.ts
